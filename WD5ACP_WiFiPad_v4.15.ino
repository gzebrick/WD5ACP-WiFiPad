...
#define VER 4.15  // Program Version Number resides here. Change this every time you save - as to a new version number.
...
uint16_t palette[] = {ILI9341_BLACK, // 0
                      ...
                      ILI9341_RED,
                      ILI9341_DARKGREY,
                      0x2100, // A dark olive green ILI9341_OLIVE,
                      ILI9341_CYAN,
                      ILI9341_MAROON,
...
int screenCount = 5;  // how many different screens do we have in rotation starting at 0
...
void loop() { // Main Loop starts here ==================================================================================================================================
  ...
  while (client.available() == 0) {
    if (millis() - timeout > KEEP_ALIVE_TIME) {
...
void drawOnAir()  {  // Screen to pop up when transmitting... // ------------------------------------------------------------------------------------------
  ...
  drawButton(0, "WD5ACP_WiFiPad", "", MenuMsg);       // This is the top menu button

  if ((millis() - onAirTime) > ON_AIR_LIMIT) { // 10 minute trasnmit timer turns ON AIR SCREEN full red
    gfx.fillBuffer(MINI_RED);
    gfx.drawString(120, 200, "ON AIR TIMER EXCEEDED");
  }

  gfx.setColor(MINI_DARKRED);
  gfx.fillRect(40, 160, 160, 60); //x, y, width, height

  gfx.setColor(MINI_WHITE);
  gfx.setTextAlignment(TEXT_ALIGN_CENTER);
  gfx.drawString(120, 180, "ON AIR");
...
void drawMenu0() { // Default screen is 8 Kenwood Host macro buttons-----------------------------------------------------------------------------------------
  ...
  drawButton(1, "", "", "");
  drawButton(2, "", "", "");
  drawButton(3, "Mute ON", "", "");
  drawButton(4, "Mute OFF", "", "");
  drawButton(5, "RIT ON", "RIT OFF", "");
  drawButton(6, "RIT", "CLEAR", "");
...
void drawButton(int Num, String Lab1, String Lab2, String Lab3) {  // Subroutine to draw buttons on screen, Lab3 only used with menu button 0 -------------------------------------
  ...
  if (Num == 0) {
    ...
    // gfx.fillRect(1, 1, 238, 54);  causes excessive WiFi traffic? VERT ODD can't use
