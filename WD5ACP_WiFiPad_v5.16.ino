...
#define VER 5.16 // Program Version Number resides here. Change this every time you save - as to a new version number.
...
/** Guy Zebrick / WD5ACP @ 2020
   ...
   5.03 Added count UP timer to ON AIR display (just displays minutes total seconds)
   5.04 restored lost connect to rig function from button 7
   5.05 removed return call in main loop that locked it out on loss of client.
   5.06 modified tickle command and on-air timer to pick up PA voltage so ON AIR no longer triggered by squelch / BUSY light off. Volts displayed
   5.07 Added code to enable backlight LED on transmit even if touchpad timed out.
   5.08 Added delay timer on boot and wait for serial shell
   ...
   5.14 Added macros for MR scan - added WiFi IP address to status menus
   5.16 5.15 tweaks to macros
*/
...
void connectWifi() {
  ...
  delay (2000); // add some delay here to let things settle...
...
void setup() { // Initial Setup start here           -------------------------------------------------------------------------------------------------------------------
  ...
  Serial.begin(115200);
  while (! Serial); // Wait for Serial to crank up
...
void loop() { // Main Loop starts here ==================================================================================================================================
  ...
  if (millis() - timeout > (KEEP_ALIVE_TIME * 1000)) {
    ...
    if (retString.startsWith("RM5")) {
      VoltString = retString; // Get the voltage reading off the meter counter (0-15v = 0-70 count)
      VoltString.remove(0, 3); // remove the RM5 from the returned string
      PaVolts = VoltString.toInt();
      // Serial.print("Volts = " + VoltString + " = " + String(PaVolts));
    }
    else {
      PaVolts = 0;
    }
    if ( PaVolts > 0) { // must be transmitting
  ...
  if (SLEEP_INTERVAL_SECS && millis() - timerPress >= (SLEEP_INTERVAL_SECS * 1000) && (screen > 1)) {               //  go to sleep
...
void drawOnAir()  {  // Screen to pop up when transmitting... // ------------------------------------------------------------------------------------------
  ...
  gfx.drawString(120, 260, "PA voltage: " + String(((PaVolts * 15.0) / 70)) + " v");
  gfx.drawString(120, 280, String((millis() - onAirTime) / 60000) + "   minutes   //   " + String((millis() - onAirTime) / 1000) + "   seconds");  // Count UP timer
  ...
  if ((millis() - onAirTime) > (ON_AIR_LIMIT * 1000)) { // 10 minute trasnmit timer turns ON AIR SCREEN full red
    ...
    gfx.drawString(120, 220, "TIMER EXCEEDED");  // Adjusted below button
  }
...
void drawConnectMenu() { //  Host Connection Menu -----------------------------------------------------------------------------------------
  ...
  drawLabelValue(4, "WiFi Strength: ", String(WiFi.RSSI()) + "dB");
  drawLabelValue(5, "WiFi IP addr: ", WiFi.localIP().toString());
  ...
  if (buttNum == 7) {
    Serial.println("connecting to Rig");
    closeHost = false;
    connectHost();
  }
  if (buttNum == 8) {
...
void drawAboutMenu() {  //----------------------------------------------------------------------------------------------------------------------------
  ...
  drawLabelValue(8, "WiFi IP addr: ", WiFi.localIP().toString());
...
void connectHost() {
  ...
  if (Host_ID == "") { // See if there's a pre-configured Host ID. If not, calculate a Kenwood TS890 style ID one based on user name and password...
    ...
    TotalPass = "##ID0" + UserString + UserPass + String(Host_USER) + String(Host_PASS); // ID0 for admin password ID1 for user password
  }
  else {
...
void checkStatus() {  // Sets the 3rd line in button 0 - either the HOST IP or a fail to connect message ------------------------------------------
  ...
  if (!closeHost) {
    ...
    MenuMsg2 = Host_IP;
    MenuMsg = MenuMsg2 + "   " + MenuMsg1;// (closeHost)
  }
  else {
    gfx.setColor(MINI_RED);
